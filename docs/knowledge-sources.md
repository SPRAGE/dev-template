...
Run `python /path/to/dev-template/tools/knowledge.py .ai/context/knowledge-sources.yaml` after creating or changing the registry, then run `ai-doctor`. The maintainer validator rejects empty registries, duplicate identifiers, endpoint-like bindings, missing citation fields, and unsafe access declarations.
