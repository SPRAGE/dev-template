...
Each skill has an explicit package allowlist, entry and total token budgets, at least five positive and five negative trigger cases, and provider/infrastructure-neutral entry text. Archives are deterministic. The catalog index estimates at 553 tokens under a 600-token cap. These checks improve maintainability and trigger hygiene; none of the new skills has yet earned a claim of better outcomes without live representative evaluation.

## Recommended Evaluation

Use `evals/behavioral/benchmark.yaml` and `tools/eval_harness.py` to record paired trials under identical model, repository, settings, and tools. The checked-in harness covers normal, hard, near-miss, and adversarial tasks; rejects unpaired, unblinded, or materially mismatched records; and reports success, regressions, safety, unnecessary actions, questions, tools, tokens, latency, cost, and grades. It does not invoke a runtime, and no live results are claimed here. Keep a default skill only when repeated external trials show gains that exceed discovery and maintenance cost.

A separate `threat-modeling` skill is intentionally not added yet. The deep reviewer already checks threat assumptions, and the benchmark now includes security-boundary, migration-recovery, and adversarial external-write cases. Promote a new skill only if live paired results show a recurring gap that the reviewer contract does not cover.
