...
**This skill writes files.** It generates or updates `.ai/instructions.md`,
`.ai/context/`, `AGENTS.md`, `CLAUDE.md`, `.mcp.json`, and `.claude/rules/`.
...
This is the provider-neutral base that Claude Code, Codex, and future agents should read.
...
### 2. AGENTS.md and CLAUDE.md Adapters

Keep these as provider adapters that point back to `.ai/`.

- `AGENTS.md` should tell Codex-compatible agents to read `.ai/instructions.md` and `.ai/context/*`.
- `CLAUDE.md` should tell Claude Code to read the same `.ai/` files and then use Claude-specific skills/settings as needed.
...
2. **.ai/instructions.md and .ai/context/**: Full content or focused diffs.
3. **AGENTS.md and CLAUDE.md**: Adapter content or focused diffs.
