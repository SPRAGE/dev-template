...
name: cc-refresh
description: >
  Audit and refresh all AI and Claude Code context — .ai/instructions.md, .ai/context,
  AGENTS.md, CLAUDE.md, auto-memory files, session history, and rules. Scans for stale content,
  scores project guidance quality (A-F grades),
  ...
  Supports --dry-run for audit-only mode and --guidance-only for .ai/AGENTS/CLAUDE focused audit.
tools: Read, Glob, Grep, Bash, Edit, Write, Agent
---

# AI Context Refresh

Audit all shared AI and Claude Code artifacts for staleness, score guidance quality, report findings,
propose fixes, and execute approved changes. This is the maintenance counterpart to `/cc-setup`.

**This skill modifies files.** It updates `.ai/`, `AGENTS.md`, `CLAUDE.md`, memory files,
and can archive old sessions. All changes require user approval.

## Modes

**Full refresh** (default): Audit all targets — `.ai/`, `AGENTS.md`, `CLAUDE.md`, memory, rules.
...
`.ai/`, `AGENTS.md`, and `CLAUDE.md` files without touching anything else.
...
> ```bash
> find . \( -name "CLAUDE.md" -o -name "AGENTS.md" -o -path "*/.ai/instructions.md" -o -path "*/.ai/context/*.md" \) 2>/dev/null | head -100
> ```
>
...
> | Codex adapter | `./AGENTS.md` | Codex-compatible entry point |
> | Claude adapter | `./CLAUDE.md` | Claude Code entry point |
