      ...
      rec {
        ...
        apps.sync-skills =
          let
            ...
            claude-agents-src = ./template/.claude/agents;
            ...
            script = pkgs.writeShellScriptBin "sync-skills" ''
              ...
              for skill_dir in "$SOURCE"/*/; do
                ...
                if [ -d "$SHARED_SKILLS_TARGET/$skill_name" ]; then
                  if ! diff -rq "$SOURCE/$skill_name" "$SHARED_SKILLS_TARGET/$skill_name" >/dev/null 2>&1; then
                    target_manifest="$SHARED_SKILLS_TARGET/$skill_name/manifest.yaml"
                    if [ -f "$target_manifest" ] && grep -Eq '^managed_by:[[:space:]]*dev-template[[:space:]]*$' "$target_manifest"; then
                      ...
                      echo "  ~ $skill_name (managed skill updated)"
                      shared_updated=$((shared_updated + 1))
                    else
                      echo "  = $skill_name (customized collision, preserved; add managed_by: dev-template to opt into template updates)"
              ...
              claude_agents_added=0
              claude_agents_updated=0
              claude_agents_unchanged=0
              claude_agents_skipped=0
              if [ -d "${claude-agents-src}" ]; then
                mkdir -p "$PWD/.claude/agents"
                echo ""
                echo "sync-skills: syncing Claude custom agents"
                for agent_file in "${claude-agents-src}"/*.md; do
                  [ -f "$agent_file" ] || continue
                  agent_name=$(basename "$agent_file")
              ...
              # Migrate only the exact retired managed provider defaults. Any
              # customized settings, MCP configuration, or hook is preserved.
              legacy_migrations=0
              if [ -f "$PWD/.claude/settings.json" ] && "$JQ" -e '
                . == {
                  "enabledPlugins": {
                  ...
                  "permissions": {
                    "allow": [
                      "Bash(git:*)", "Bash(nix:*)", "Bash(npx:*)", "Bash(cargo:*)", "Bash(uv:*)",
                      "mcp__context7__resolve-library-id", "mcp__context7__query-docs"
                    ],
                    "deny": [
                      "Edit(//.env)", "Edit(//.env.*)", "Read(//.env)", "Read(//.env.*)",
                      "Edit(//.git/**)", "Bash(git push --force:*)", "Bash(sudo:*)", "Bash(curl:*)",
                      "Bash(wget:*)", "Bash(ssh:*)", "Bash(scp:*)", "Bash(nix-store --delete:*)"
                    ]
                  },
                  "hooks": {
                ...
                or . == {
                  "permissions": {
                    "allow": ["Bash(git:*)", "Bash(nix:*)", "Bash(npx:*)", "Bash(cargo:*)", "Bash(uv:*)"],
                    "deny": [
                      "Edit(//.env)", "Edit(//.env.*)", "Read(//.env)", "Read(//.env.*)",
                      "Edit(//.git/**)", "Bash(git push --force:*)", "Bash(sudo:*)", "Bash(curl:*)",
                      "Bash(wget:*)", "Bash(ssh:*)", "Bash(scp:*)", "Bash(nix-store --delete:*)"
              ...
              ' "$PWD/.claude/settings.json" >/dev/null 2>&1; then
                cp -L "${settings-src}" "$PWD/.claude/settings.json"
                chmod u+w "$PWD/.claude/settings.json"
                echo "  ~ .claude/settings.json (retired managed defaults migrated)"
                legacy_migrations=$((legacy_migrations + 1))
              fi

              if [ -f "$PWD/.mcp.json" ] && "$JQ" -e '
                . == {
                  "mcpServers": {
                    "context7": {
                      "type": "stdio",
              ...
              ' "$PWD/.mcp.json" >/dev/null 2>&1; then
                cp -L "${mcp-src}" "$PWD/.mcp.json"
                chmod u+w "$PWD/.mcp.json"
                echo "  ~ .mcp.json (retired managed Context7 server removed)"
                legacy_migrations=$((legacy_migrations + 1))
              fi

              legacy_session_hook="$PWD/.claude/hooks/session-start.sh"
              if [ -f "$legacy_session_hook" ]; then
                legacy_hook_hash=$("${pkgs.coreutils}/bin/sha256sum" "$legacy_session_hook" | "${pkgs.coreutils}/bin/cut" -d ' ' -f 1)
                if [ "$legacy_hook_hash" = "54b9e5bb7e4c4eb87ebdf01dc7ca7130e7fb29e463674aed991fb39679422918" ]; then
                  if [ ! -f "$PWD/.claude/settings.json" ] || ! grep -Fq '.claude/hooks/session-start.sh' "$PWD/.claude/settings.json"; then
                    rm -f "$legacy_session_hook"
                    echo "  - .claude/hooks/session-start.sh (retired managed hook)"
                    legacy_migrations=$((legacy_migrations + 1))
                  else
                    echo "  = .claude/hooks/session-start.sh (referenced by customized settings, preserved)"
                  fi
                fi
              fi

              # Add managed hooks, but do not overwrite project-customized hooks.
                ...
                for hook_file in "$HOOKS_SOURCE"/*; do
                  ...
                  if [ -f "$HOOKS_TARGET/$hook_name" ]; then
                    if diff -q "$HOOKS_SOURCE/$hook_name" "$HOOKS_TARGET/$hook_name" >/dev/null 2>&1; then
                      ...
                      echo "  = $hook_name (customized, not overwriting)"
              ...
              # Seed the neutral runtime specification without replacing project customization.
              AI_SOURCE="${ai-src}"
              AI_TARGET="$PWD/.ai"
              ai_added=0
              ai_skipped=0
              if [ -d "$AI_SOURCE" ]; then
                ...
                for spec_path in project.yaml policy.yaml capabilities agents evals generators catalog tools; do
                  ...
                  elif [ -e "$AI_SOURCE/$spec_path" ]; then
                    cp -rL "$AI_SOURCE/$spec_path" "$AI_TARGET/$spec_path"
                    chmod -R u+w "$AI_TARGET/$spec_path"
                    echo "  + .ai/$spec_path (added)"
                    ai_added=$((ai_added + 1))
                  fi
                done

                compiler_source="$AI_SOURCE/generators/compile.py"
                compiler_target="$AI_TARGET/generators/compile.py"
                if [ -f "$compiler_source" ]; then
                  mkdir -p "$(dirname "$compiler_target")"
                  if [ ! -f "$compiler_target" ]; then
                    cp -L "$compiler_source" "$compiler_target"
                    chmod u+wx "$compiler_target"
                    echo "  + .ai/generators/compile.py (added)"
                    ai_added=$((ai_added + 1))
              ...
              is_managed_adapter() {
                file="$1"
                grep -q "Generated from .ai/" "$file" 2>/dev/null ||
        ...
        apps.fresh-start =
          let
            ...
            python-flake-src = ./templates/python/flake.nix;
            rust-flake-src = ./templates/rust/flake.nix;
            python-ai-src = ./templates/python/.ai;
            rust-ai-src = ./templates/rust/.ai;
            ai-md-src = ./template/AI.md;
            python-ai-md-src = ./templates/python/AI.md;
            rust-ai-md-src = ./templates/rust/AI.md;
            claude-md-src = ./template/CLAUDE.md;
            agents-md-src = ./template/AGENTS.md;
            codex-md-src = ./template/CODEX.md;
            script = pkgs.writeShellScriptBin "fresh-start" ''
              ...
              flake_kind=base
              if [ -f "$PWD/flake.nix" ] && grep -q "rust-overlay" "$PWD/flake.nix"; then
                flake_kind=rust
              elif [ -f "$PWD/Cargo.toml" ]; then
                flake_kind=rust
              elif [ -f "$PWD/flake.nix" ] && grep -Eq "python313|pkgs\.uv" "$PWD/flake.nix"; then
                flake_kind=python
              elif [ -f "$PWD/pyproject.toml" ]; then
                flake_kind=python
              fi
              selected_ai_src="${ai-src}"
              selected_ai_md_src="${ai-md-src}"
              case "$flake_kind" in
                rust)
                  selected_ai_src="${rust-ai-src}"
                  selected_ai_md_src="${rust-ai-md-src}"
                  ;;
                python)
                  selected_ai_src="${python-ai-src}"
                  selected_ai_md_src="${python-ai-md-src}"
                  ;;
              esac
              echo "  Targets:"
              ...
              [ -f "$PWD/.claude/settings.local.json" ] && echo "    .claude/settings.local.json (preserved)"
              for local_dir in local tmp sessions logs; do
                [ -e "$PWD/.ai/$local_dir" ] && echo "    .ai/$local_dir/ (preserved)"
              done
              ...
              if [ -d "$PWD/.ai" ]; then
                for local_dir in local tmp sessions logs; do
                  if [ -e "$PWD/.ai/$local_dir" ]; then
                    mkdir -p "$preserve_dir/.ai"
                    mv "$PWD/.ai/$local_dir" "$preserve_dir/.ai/$local_dir"
                  fi
                done
                rm -rf "$PWD/.ai"
                echo "  - .ai/"
              ...
              if [ -d "$PWD/.claude" ]; then
                if [ -e "$PWD/.claude/settings.local.json" ]; then
                  mkdir -p "$preserve_dir/.claude"
                  mv "$PWD/.claude/settings.local.json" "$preserve_dir/.claude/settings.local.json"
                fi
                rm -rf "$PWD/.claude"
                echo "  - .claude/"
              fi
              ...
              # Nix flake
              case "$flake_kind" in
                rust) cp -L "${rust-flake-src}" "$PWD/flake.nix" ;;
                python) cp -L "${python-flake-src}" "$PWD/flake.nix" ;;
                *) cp -L "${flake-src}" "$PWD/flake.nix" ;;
              esac
              chmod u+w "$PWD/flake.nix"
              echo "  + flake.nix ($flake_kind template)"

              # Provider-neutral AI context
              mkdir -p "$PWD/.ai"
              cp -rL "$selected_ai_src/." "$PWD/.ai/"
              chmod -R u+w "$PWD/.ai"
              if [ -d "$preserve_dir/.ai" ]; then
                for local_dir in "$preserve_dir/.ai"/*; do
                  [ -e "$local_dir" ] || continue
                  mv "$local_dir" "$PWD/.ai/$(basename "$local_dir")"
                done
              fi
              echo "  + .ai/"
              ...
              cp -L "${settings-src}" "$PWD/.claude/settings.json"
              chmod u+w "$PWD/.claude/settings.json"
              if [ -e "$preserve_dir/.claude/settings.local.json" ]; then
                mv "$preserve_dir/.claude/settings.local.json" "$PWD/.claude/settings.local.json"
              fi
              ...
              mkdir -p "$PWD/.claude/agents"
              cp -rL "${claude-agents-src}/." "$PWD/.claude/agents/"
              chmod -R u+w "$PWD/.claude/agents"
              claude_agent_count=$(ls "$PWD/.claude/agents"/*.md 2>/dev/null | wc -l)
              echo "  + .claude/agents/ ($claude_agent_count agents)"
              ...
              # AI.md
              cp -L "$selected_ai_md_src" "$PWD/AI.md"
              chmod u+w "$PWD/AI.md"
              echo "  + AI.md"
            ...
            script = pkgs.writeShellScriptBin "onboard" ''
              ...
              for spec_path in project.yaml policy.yaml methodology.md capabilities agents evals generators catalog tools; do
                if [ -e "$PWD/.ai/$spec_path" ]; then
                  ...
                  cp -rL "${ai-src}/$spec_path" "$PWD/.ai/$spec_path"
                  chmod -R u+w "$PWD/.ai/$spec_path"
                  echo "  + .ai/$spec_path"
                fi
              done
              ...
              for skill_dir in "${skills-src}"/*/; do
                ...
                if [ -d "$PWD/.ai/skills/$skill_name" ]; then
                  if ! diff -rq "$skill_dir" "$PWD/.ai/skills/$skill_name" >/dev/null 2>&1; then
                    target_manifest="$PWD/.ai/skills/$skill_name/manifest.yaml"
                    if [ -f "$target_manifest" ] && grep -Eq '^managed_by:[[:space:]]*dev-template[[:space:]]*$' "$target_manifest"; then
                      rm -rf "$PWD/.ai/skills/$skill_name"
                      cp -rL "$skill_dir" "$PWD/.ai/skills/$skill_name"
                      chmod -R u+w "$PWD/.ai/skills/$skill_name"
                      echo "  ~ .ai/skills/$skill_name (managed skill updated)"
                    else
                      echo "  = .ai/skills/$skill_name (customized collision, preserved)"
                    fi
                  else
              ...
              for f in "${hooks-src}"/*; do
                ...
                fname=$(basename "$f")
                [ "$fname" = ".gitkeep" ] && continue
                if [ -f "$PWD/.claude/hooks/$fname" ]; then
                  if cmp -s "$f" "$PWD/.claude/hooks/$fname"; then
                    echo "  = .claude/hooks/$fname (up to date)"
                  else
                    echo "  = .claude/hooks/$fname (customized, skipped)"
                  fi
                else
                  cp -L "$f" "$PWD/.claude/hooks/$fname"
                  chmod u+w "$PWD/.claude/hooks/$fname"
              ...
              for agent_file in "${claude-agents-src}"/*.md; do
                [ -f "$agent_file" ] || continue
                agent_name=$(basename "$agent_file")
                if [ -f "$PWD/.claude/agents/$agent_name" ]; then
                  echo "  = .claude/agents/$agent_name (already exists, skipped)"
                else
                  cp -L "$agent_file" "$PWD/.claude/agents/$agent_name"
                  chmod u+w "$PWD/.claude/agents/$agent_name"
          ...
          let
            ...
            script = pkgs.writeShellScriptBin "ai-doctor" ''
              ...
              if [ -d .ai ]; then
                ...
                for path in project.yaml policy.yaml methodology.md capabilities/map.yaml capabilities/profiles.yaml capabilities/runtimes/codex.yaml capabilities/runtimes/claude.yaml evals/contract-scenarios.yaml generators/compile.py catalog/index.yaml tools/skillctl.py; do
                  if [ -e ".ai/$path" ]; then
                    ...
                    fail ".ai/$path missing"
                  fi
                done
                if [ -d .ai/agents ] && [ "$(find .ai/agents -maxdepth 1 -name '*.yaml' | wc -l)" -gt 0 ]; then
                  pass ".ai/agents contains neutral agent contracts"
                else
                  fail ".ai/agents missing neutral agent contracts"
                fi
                ...
                if [ -n "$shared_skills_root" ]; then
                  ...
                  for skill_dir in "$shared_skills_root"/*; do
                    ...
                    if [ -f "$skill_dir/manifest.yaml" ]; then
                      pass "$skill_dir has manifest.yaml"
                    else
                      fail "$skill_dir missing manifest.yaml"
                    fi
                  done
              ...
              if [ -d .claude ]; then
                ...
                if [ -d .claude/agents ] && [ "$(find .claude/agents -maxdepth 1 -name '*.md' | wc -l)" -gt 0 ]; then
                  pass ".claude/agents contains generated agents"
                else
                  fail ".claude/agents missing generated agents"
                fi
              else
                warn ".claude/ missing"
              fi
              ...
              if [ -x .ai/generators/compile.py ]; then
                if "$PYTHON" .ai/generators/compile.py --root . --check >/dev/null 2>&1; then
                  pass "generated runtime artifacts match the neutral spec and token budgets"
                else
                  fail "generated runtime artifacts are stale or the neutral spec is invalid"
                fi
              fi
