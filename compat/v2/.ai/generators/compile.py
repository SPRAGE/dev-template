...
def render_adapter(runtime: str, runtime_spec: dict) -> str:
    shared = "Read `AI.md` and `.ai/instructions.md`; load further guidance only when they route to it."
    ...
    fallback_note = "If a needed role is unavailable, work inline and report the limitation."
    if runtime == "codex":
        notes = (
            "Skills: `.agents/skills/`. Roles: `.codex/agents/`."
        )
        return f"# Codex Adapter\n\n<!-- {GENERATED_MARKER} -->\n\n{shared}\n\n{notes} {fallback_note}\n"
    notes = (
        "Skills: `.claude/skills/`. Roles: `.claude/agents/`. "
    )
...
When present, read `{context['conditional']['conventions']}` before edits/review; `{context['conditional']['architecture']}` for architecture; `{context['conditional']['knowledge']}` when using configured domain sources. Decisions, active context, and skill bodies are task-specific.

For {inspect_intents}, inspect and report only. For {change_intents}, edit and validate. Obtain authorization for {confirm_before} unless already authorized; ask only for an unresolved {question_trigger}.

Stay in the {scope_boundary}. Access another repository only when {scope_authorization}. If code is absent, {absent_scope_action}.

Direct requires {direct}; act and verify. Read `.ai/methodology.md` for Planned ({planned}) or Hard ({hard}) work. Hard takes precedence. File count alone does not change the route.

Preserve {preserve}, including provider-local settings and local/tmp directories. Never weaken behavior or tests to pass checks. Report {words(completion['statuses'])} with evidence; unfinished work needs blockers and a next action.
"""
...
Keep planning and integration in the primary context. For Hard work, define stop/rollback gates and obtain independent deep review.

Ground consequential decisions in repository evidence, relevant domain rules, and current authoritative sources. Retain only goals, invariants, boundaries, and unknowns that affect acceptance. Missing knowledge blocks only dependent work.

Make one plan with file scopes, success criteria, risks, and checks. Infer reversible details and continue ready work; ask only for decisions evidence cannot settle. Persist a plan only when requested.

Delegate only a bounded, independent task when isolation or parallelism repays the handoff cost. Supply {required}; stop on {stop_on}. Use fast reads, balanced implementation, and independent deep review when risk warrants. Do tightly coupled work inline.

Re-read the integrated diff, check invariants, and run the cheapest meaningful verification. Stop after sufficient evidence unless new failures or concerns justify more work. Store only recurring verified facts in project context.
"""
...
def validate_skill_catalog(ai: Path, token_budget: int) -> None:
    catalog = ai / "catalog"
    # Legacy/project-owned catalogs remain valid, but new templates ship none.
...
def context_metrics(root: Path, project: dict, generated: dict[Path, str], agents: list[dict]) -> dict:
    """Conservative static estimates, including runtime discovery descriptions."""
    def content(path: Path) -> str:
        return generated[path] if path in generated else path.read_text(encoding="utf-8")

    guidance = sum(estimate_tokens(content(root / relative)) for relative in project["context"]["always"])
    guidance += max(estimate_tokens(content(root / name)) for name in ("AGENTS.md", "CLAUDE.md"))
    role_catalog = sum(estimate_tokens(f"{agent['mission']} Use when: {agent['when_to_use']}") for agent in agents)
    descriptions = 0
    entries = {}
    for path in sorted((root / ".ai/skills").glob("*/SKILL.md")):
        text = path.read_text(encoding="utf-8")
        match = re.match(r"^---\n(.*?)\n---\n", text, re.DOTALL)
        if not match:
            raise ValueError(f"{path} has invalid frontmatter")
        metadata = yaml.safe_load(match.group(1))
        descriptions += estimate_tokens(str(metadata.get("description", "")))
        entries[path.parent.name] = estimate_tokens(text)
    startup = guidance + role_catalog + descriptions
    index = root / ".ai/catalog/index.yaml"
    return {
        "guidance_tokens": guidance,
        "startup_tokens": startup,
        "planned_tokens": startup + estimate_tokens(content(root / ".ai/methodology.md")),
        "role_catalog_tokens": role_catalog,
        "skill_description_tokens": descriptions,
        "specialist_index_tokens": estimate_tokens(index.read_text(encoding="utf-8")) if index.is_file() else 0,
        "skill_entry_tokens": entries,
    }
...
def build_outputs(root: Path) -> dict[Path, str]:
    ...
    metrics = context_metrics(root, project, generated, agents)
    runtime_total = metrics["startup_tokens"]
    ...
    agent_catalog_total = metrics["role_catalog_tokens"]
    ...
    planned_total = metrics["planned_tokens"]
...
def main() -> int:
    ...
    parser.add_argument("--stats", action="store_true", help="print static context estimates as JSON without writing files")
    parser.add_argument(
        "--preserve-existing",
        action="store_true",
    ...
    if args.stats:
        project = load_yaml(root / ".ai/project.yaml")
        agents = [load_yaml(path) for path in sorted((root / ".ai/agents").glob("*.yaml"))]
        print(json.dumps(context_metrics(root, project, generated, agents), indent=2))
        if not args.check:
            return 0
    stale: list[Path] = []
    for path, text in generated.items():
